import React, {
  ...
  import { Props, GetResults, Value, FilterOptionsResult } from 'payload/dist/admin/components/forms/field-types/Relationship/types';
  ...
  const CustomRelationship: React.FC<Props> = (props) => {
	const {
	  relationTo,
		...
		if (idsToLoad.length > 0) {
		  ...
		  if (!errorLoading) {
			...
			if (response.ok) {
			  ...
			  console.log('Got the relationTo docs from the database');
			  console.log(docs);
			}
	...
	console.log("About to run `findOptionsByValue` where the error occurs because options is empty");
  ...
  export default withCondition(CustomRelationship);